...
import com.javierrebollo.basearch.utils.Tracker.STATE_ERROR
...
class Foreground : Application.ActivityLifecycleCallbacks {
    ...
    override fun onActivityResumed(activity: Activity) {
        ...
        if (wasBackground) {
            Tracker.trackIsInForeground(true, TAG)
            for (l in listeners) {
                try {
                    l.onBecameForeground()
                } catch (exc: Exception) {
                    Tracker.logError(STATE_ERROR, TAG, "Listener failed!", exc)
                }
            }
        } else {
            Tracker.logDebug(TAG, "still foreground")
        }
    }
    ...
    override fun onActivityPaused(activity: Activity) {
        ...
        handler.postDelayed({
            if (isForeground && paused) {
                isForeground = false
                Tracker.trackIsInForeground(false, TAG)
                for (l in listeners) {
                    try {
